...
test('Windows Cuda network versions in descending order', async () => {
...
test('Windows network Cuda version to URL map contains valid URLs', async () => {
  for (const version of WindowsLinks.Instance.getAvailableNetworkCudaVersions()) {
    const url: URL = WindowsLinks.Instance.getNetworkURLFromCudaVersion(version)
    expect(url).toBeInstanceOf(URL)
  }
})

test('There is at least windows network 1 version url pair', async () => {
  expect(
